...
async function init(viewPath) {
    ...
    blackboard.registerListener(MSG_SHAPE_REMOVED, message => 
        blackboard.broadcastMessage(MSG_MODEL_NODES_MODIFIED,
    ...
    blackboard.registerListener(MSG_MODEL_ADD_NODE, message => {
        ID_CACHE[message.id] = message.properties;
        ...
        blackboard.broadcastMessage(MSG_ADD_SHAPE, {
            name: _generateShapeName(message.nodeName), id: message.id,
    ...
    blackboard.registerListener(MSG_FILE_UPLOADED, async message => {
        await reset(); blackboard.broadcastMessage(MSG_MODEL_LOAD_MODEL,
...
function _validateConnection(message) {
    ...
    for (const validator of validators) if (!validator({
        sourceName: message.sourceName, targetName: message.targetName,
        sourceID: message.sourceID, targetID: message.targetID
    })) return false;
    return true;
}
