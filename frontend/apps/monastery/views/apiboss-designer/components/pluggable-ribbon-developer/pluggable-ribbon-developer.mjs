 ...
 */
 ...
 async function elementConnected(element) {
     ...
     pluggable_ribbon_developer.setData(element.id, data);
 }
 
 async function elementRendered(element) {
     const shadowRoot = pluggable_ribbon_developer.getShadowRootByHostId(element.id);
     for (const pluginName in pluggable_ribbon_developer.extensions[element.id||"null"]) // attach shadowRoots to the plugins
         pluggable_ribbon_developer.extensions[element.id||"null"][pluginName].shadowRoot = shadowRoot;
 }
 ...
 async function _instantiatePlugins(element) {
     ...
     const data = {plugins:[]}; pluggable_ribbon_developer.extensions = pluggable_ribbon_developer.extensions||{}; pluggable_ribbon_developer.extensions[element.id||"null"] = {};
     ...
     for (const plugin of plugins) {
         ...
         if (pluginModule && await pluginModule.init(`${COMPONENT_PATH}/${element.id}/${plugin}`)) {
             pluggable_ribbon_developer.extensions[element.id][plugin] = pluginModule;
 ...
 export const pluggable_ribbon_developer = {trueWebComponentMode: true, elementConnected, elementRendered}
 monkshu_component.register("pluggable-ribbon-developer", `${COMPONENT_PATH}/pluggable-ribbon-developer.html`, pluggable_ribbon_developer);