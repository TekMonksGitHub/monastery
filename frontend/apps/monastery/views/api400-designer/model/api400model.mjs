...
function loadModel(jsonModel) {
    ...
    if (!(api400modelObj.apicl)) {LOG.error(`Bad Monkruls model, not in right format.`); return;}
    ...
    for (const bundle of api400modelObj.rule_bundles) if (!bundle.commands.decisiontable) 
        for (const rule of bundle.commands) if (rule.dependencies) for (const dependency of rule.dependencies) connectNodes(dependency, rule.id);
...
function _nodeModified(nodeName, id, properties) {
    ...
    for (const key in properties) { // transfer the new properties, CSVs need the CSV scheme added
        if (key == "decisiontable") {   // decision table must be CSV
            ...
            idCache[id].data_raw = properties[key]; 
        } else if (key.includes("parameter") && nodeName == "strapi") { 
            console.log(api400modelObj.apicl);
            if(properties[key]!='')
            console.log(_addParamInStrapi(properties[key]))
            // idCache[id][key] = properties.type == "CSV" ? CSVSCHEME+properties.data : _tryJSONParse(properties.data);
            // idCache[id].data_raw = properties[key]; 
...
function _addParamInStrapi(param){
    let pos = api400modelObj.apicl.findIndex(element => element.includes("STRAPI"));
    api400modelObj.apicl[pos] = api400modelObj.apicl[pos].replace(`)`,`&${param} )`); 
} 
